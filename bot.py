...
class MaksBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        print(f"✅ Maks Reps 2.0: Wszystkie systemy są online.")
...
async def panel(interaction: discord.Interaction, typ: app_commands.Choice[str]):
    ...
    if val == "tickets":
        ...
        embed = discord.Embed(title="☀️ MAKS REPS × WYBIERZ PINGI", description="🎁 **Ping Promocje**\n🎬 **Ping TikTok**", color=discord.Color.red())
        await interaction.response.send_message(embed=embed, view=RoleViewAll(ROLE_TIKTOK_ID, ROLE_PROMOCJE_ID))
    elif val == "roles_promo":
        embed = discord.Embed(title="☀️ MAKS.R3PS X PROMOCJE", description="🎁 **Ping Promocje**", color=discord.Color.red())
        await interaction.response.send_message(embed=embed, view=RoleViewPromo(ROLE_PROMOCJE_ID))
    elif val == "roles_tiktok":
        embed = discord.Embed(title="☀️ MAKS.R3PS X FILMY", description="🎬 **Ping TikTok**", color=discord.Color.red())
...
@app_commands.describe(tytul="Wpisz tytuł", opis="Wpisz zasady", czas="Np. 1h", zwyciezcy="Liczba osób", kolor="HEX")
...
# --- NOWA KOMENDA /GREROLL (DODANA NA KONIEC) ---
