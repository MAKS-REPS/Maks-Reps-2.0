...
ID_KATEGORII_TICKETOW = 0 # Tutaj wkleisz ID po użyciu !id
MAKS_BLUE = 0x3498db

intents = discord.Intents.all()

# --- KLASA MENU WYBORU (Z DODANYM CUSTOM_ID) ---
class TicketMenu(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(
            ...
            max_values=1, 
            options=options,
            custom_id="maks_reps_ticket_select" 
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if ID_KATEGORII_TICKETOW == 0 or category is None:
            await interaction.response.send_message("❌ Błąd: ID kategorii nie jest ustawione! Użyj !id.", ephemeral=True)
            return
...
# --- WIDOK (Z WYŁĄCZONYM TIMEOUTEM) ---
class TicketView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None) # To musi być None dla trwałych paneli
...
class MaksBot(commands.Bot):
    ...
    async def setup_hook(self):
        # To teraz zadziała, bo dodaliśmy custom_id w Menu
...
async def id(ctx):
    if ctx.channel.category:
        await ctx.send(f"🆔 ID kategorii: `{ctx.channel.category.id}`")
    else:
        await ctx.send("❌ Brak kategorii!")
